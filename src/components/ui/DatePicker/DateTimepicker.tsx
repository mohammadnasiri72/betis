  ...
  (props, ref) => {
    ...
    useEffect(() => {
      ...
      if (value instanceof Date && !focused) {
        setInputState(jalaliMoment(value).format('YYYY/MM/DD HH:mm'))
      }
    ...
    const handleValueChange = (date: Date) => {
      ...
      if (!value && !closePickerOnChange) {
        setInputState(
          jalaliMoment(date).format('YYYY/MM/DD HH:mm')
        );
      }
      closePickerOnChange &&
        setInputState(date);
    ...
    const parseDate = (date: string) =>
      jalaliMoment(date, dateFormat, finalLocale).toDate();
    ...
    const handleTimeChange = (time: Date | null) => {
      if (_value instanceof Date && time instanceof Date) {
        ...
        if (!value && !closePickerOnChange) {
          setInputState(jalaliMoment(newDateTime).format('YYYY/MM/DD HH:mm'))
        }
    ...
    const handleOk = () => {
      if (_value instanceof Date) {
        setInputState(jalaliMoment(_value).format('YYYY/MM/DD HH:mm'));
        closeDropdown();
        window.setTimeout(() => inputRef.current?.focus(), 0);
        onChange?.(_value as Date | null);
      } else {
        // _value is either null or undefined
    ...
    return (
      ...
      >
        <Calendar
          ...
          value={
            _value instanceof Date ? _value : (_value && jalaliMoment(_value).toDate())
          }

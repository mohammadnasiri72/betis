...
export default function ModalNewBoardNotice({ setFlag, valBuilding, type }) {
  ...
  const handleClickOpen = () => {
    setOpen(true);
    setValReceiversType(listReceiversType[0]);
  };
    ...
    if (open) {
      ...
      axios
        ...
        .then((res) => {
          ...
          setValReceiversType(res.data[0]);
        })
        .catch((err) => {
          setIsLoading(false);
        });
  ...
  // set new notice
  const setNewBoardNoticeHandler = () => {
    if (title && body) {
      const data = {
      ...
      setIsLoading(true);
      axios
        .post(`${mainDomain}/api/BoardNotice/Add`, data, {
          headers: {
            Authorization: `Bearer ${localStorage.getItem('token')}`,
          },
        })
      ...
      >
        ...
        <DialogActions>
          <Button
            ...
            variant="contained"
            autoFocus
            onClick={setNewBoardNoticeHandler}
          >
            ثبت
          </Button>
        </DialogActions>

...
function ModalEditRealEstate({ serviceHome, setFlag, typeRealEstate, subjectsRealEstate, listUnit }) {
  ...
  useEffect(() => {
    if (serviceHome.id) {
      const typeEdit = Object.keys(typeRealEstate).find((key) => typeRealEstate[key] === serviceHome.type);
      const subjectsEdit = Object.keys(subjectsRealEstate).find(
        (key) => subjectsRealEstate[key] === serviceHome.subject
      );

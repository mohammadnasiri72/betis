...
export default function DoneChargeHandler({
  valBuilding,
  valCharge,
  valTerm,
  unitIdList,
  ...
  const handleClickOpen = () => {
    ...
    } else if (!toEnglishNumber(dueDate).match(paternDate)) {
      Toast.fire({
        icon: 'error',
        text: 'لطفا تاریخ سر رسید را به درستی وارد کنید',
        customClass: {
          container: 'toast-modal',
        },
      });
        ...
        <button
          ...
          className={
            listReportDebt.length > 0 || listCharge.filter((ev) => ev.typeId === 1).length === 0
              ? 'flex w-full items-center bg-slate-800 duration-300 opacity-50 px-3 py-2 rounded-lg text-white'
        ...
        <DialogContent dividers>
          <div className="flex items-center">
            ...
            <div className="text-start px-3">
              <h4 className={themeMode === 'dark' ? 'text-white' : 'text-black'}>اعمال شارژ</h4>

...
export default function ModalCancelRequest({ setFlag, e }) {
  ...
  const cancelRequestHandler = () => {
    ...
    axios
      .post(
        `${mainDomain}/api/Summon/Cancel?id=${e.id}`,
        {},
        {
          headers: {
  ...
  return (
    <>
      <button
        ...
        className="bg-red-50 text-red-600 text-xs rounded-lg px-3 py-2 hover:bg-red-100 duration-300 font-semibold"
      >
        لغو درخواست
      </button>
      <BootstrapDialog
        ...
        <DialogContent dividers>
          <div className="flex items-center">
            ...
            <div className="text-start px-3">
              <h4 style={{ color: themeMode === 'dark' ? '#fff' : '#000' }}>لغو درخواست</h4>
              <p className={themeMode === 'dark' ? 'text-[#fff8]' : 'text-[#0008]'}>آیا از لغو درخواست مطمئن هستید؟</p>
            </div>
          </div>
        </DialogContent>
        <DialogActions sx={{ backgroundColor: themeMode === 'dark' ? '#212b46' : '#f3f4f6' }}>
          ...
          <button
            ...
            <div className="px-2">
              ...
              >
                {!isLoading && <span className="whitespace-nowrap">لغو</span>}
                {isLoading && (

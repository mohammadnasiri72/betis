...
export default function ShowOrderActive({ accountResident }) {
  ...
  return (
    <>
      {listMyMenu.map((order) => (
        <div
          key={order.id}
          className={
            themeMode === 'dark' ? 'rounded-xl border mt-2 bg-slate-700 p-3' : 'rounded-xl border mt-2 bg-[#e7e7e7] p-3'
          }
        >
          <div className="flex flex-col items-start">
            ...
            <div className="flex justify-between w-full flex-wrap">
              <div className="flex items-center gap-2 mt-1">
                ...
                className="bg-[#72dc9b] px-3 rounded-lg text-xs font-semibold py-1">{order.paymentStatus}</span>
                <span

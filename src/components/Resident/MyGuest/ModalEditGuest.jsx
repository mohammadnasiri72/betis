...
export default function ModalEditGuest({ setFlag, accountResident, handleCloseMenu, guest }) {
  ...
  const dateEdit = farvardin.solarToGregorian(
    Number(guest.dateArrivalFa.slice(0, 4)),
    Number(guest.dateArrivalFa.slice(5, 7)),
    Number(guest.dateArrivalFa.slice(8, 10)),
    'object'
  );
  useEffect(() => {
    const date = new Date();
    date.setFullYear(dateEdit.year);
    date.setMonth(dateEdit.month - 1);
    date.setDate(dateEdit.day);
    setDate(date);
  }, []);

  const setDetailGuest = () => {
    setNameGuest(guest.name);
    setNumberGuest(guest.contactNumber ? guest.contactNumber : '');
    setValDate(guest.dateArrivalFa);
    if (guest.startArrivalTime) {
      setIsShowTime(true);
      setStartTime(10);
    }
    if (guest.licensePlate.length === 7) {
      setValTypeVehicle(2);
    }
    if (guest.licensePlate.length === 8) {
      setValTypeVehicle(1);
    }
  ...
  const editGuestHandler = () => {
    ...
    if (nameGuest && date) {
      if (
        ...
        setIsLoading(true);
        axios
          .put(`${mainDomain}/api/Guest/Update`, data, {
            headers: {
          ...
          .then(() => {
            ...
            Toast.fire({
              icon: 'success',
              text: 'مهمان با موفقیت ویرایش شد',
              customClass: {
      ...
      >
        ...
        <DialogContent dividers className="sm:min-w-[600px]">
          <div className="flex flex-wrap items-start">
            ...
            {/* select from time */}
            <div className="sm:w-1/3 relative w-full px-1">
              <DatePicker
            ...
            {isShowTime && (
              <div className="sm:w-2/3 w-11/12 mx-auto mt-1">
                <SelectRangHour
                  ...
                  startTime={guest.startArrivalTime}
                  endTime={guest.endArrivalTime}
                />
              </div>
            )}
          </div>

...
export default function ModalDeleteServiceMenu({ menu, setFlag, setIsLoading }) {
  ...
  //   delete handler
  const deleteServiceTimeHandler = () => {
    setIsLoading(true);
    axios
      .delete(`${mainDomain}/api/ServiceMenu/Delete/${menu.id}`, {
        headers: {
  ...
  return (
    <>
      <MenuItem className="w-full" onClick={handleClickOpen}>
        <div className="flex items-center text-sm justify-center w-full">
          <MdOutlineDelete className="text-white" />
          <Stack className="px-1 text-white">حذف</Stack>
        </div>
      </MenuItem>
      <BootstrapDialog
        ...
        <DialogContent dividers>
          <div className="flex items-center">
            ...
            <div className="text-start px-3">
              <h4 style={{ color: themeMode === 'dark' ? '#fff' : '#000' }}>حذف منو</h4>

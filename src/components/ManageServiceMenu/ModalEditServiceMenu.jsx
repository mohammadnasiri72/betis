...
export default function ModalEditServiceMenu({ setFlag, menu, listService, valBuilding }) {
  ...
  useEffect(() => {
    if (menu && listService.length > 0) {
      setValService(listService.find((e) => e.id === menu.serviceId));
      setNameCategory(menu.category);
      setName(menu.title);
      setPrice(numberWithCommas(menu.price));
      setDescription(menu.description);
      setIsActive(menu.isActive);
      setFileAtt(menu.image);
      setAvatarTemporary(mainDomain + menu.imageSrc);
      if (menu.inventory === -1) {
  ...
  const editServiceMenu = () => {
    if (name && price && nameCategory) {
      ...
      setIsLoading(true);
      axios
        .put(`${mainDomain}/api/ServiceMenu/Update`, data, {
          headers: {
            Authorization: `Bearer ${localStorage.getItem('token')}`,
          },
        })
  ...
  return (
    <>
      <MenuItem className="w-full" onClick={handleClickOpen}>
        <div className="flex items-center text-xs w-full justify-center">
          <SettingsIcon sx={{ fontSize: '1rem' }} className="text-white" />
          <span className="px-1 text-white">ویرایش</span>
        </div>
      </MenuItem>
      <BootstrapDialog
        ...
        <div>
          ...
          <DialogContent dividers>
            ...
            <div className="flex flex-wrap mt-3">
              ...
              <div className="sm:w-1/5 w-full px-2 flex flex-col">

...
import ModalDoneMenu from './ModalDoneMenu';
import ModalProcessMenu from './ModalProcessMenu';

export default function BoxOrder({ menu, setFlag }) {
      ...
      <div
        ...
        className={
          menu.statusId === 1
            ...
            : menu.statusId === 3
            ? 'duration-1000 border-[5px] border-emerald-500'
            : 'duration-1000 border-[5px] border-red-500'
        }
      >
        <Card>
          <div className="flex justify-between px-3 py-3 ">
            <div className="flex items-start">
              ...
              <div className="flex flex-col items-start justify-center px-1 ">
                <span className="font-semibold px-3">{menu.unitTitle}</span>
                <span className="px-3 text-sm">{menu.serviceTitle}</span>
            ...
            <div className="flex flex-col items-center">
              {menu.isExpiredDueDate && (
                <div className="text-xs bg-red-100 text-red-600 rounded-full px-2 py-1 mb-2 flex items-center">
          ...
          <div className="flex justify-between items-end w-full p-2">
            <button
              ...
              className="px-2 py-1 rounded-lg text-teal-500 flex items-center duration-300 hover:text-teal-600 text-sm whitespace-nowrap"
            >
              <span className="mt-1 whitespace-nowrap">مشاهده جزئیات سفارش</span>
              <ExpandMoreIcon
            ...
            <div className="flex justify-center items-center flex-wrap sm:flex-nowrap">
              {checkClaims(url.pathname, 'put') && (
                <div className="md:w-1/2 w-full px-1">
                  {' '}
                  {menu.statusId === 2 && <ModalDoneMenu setFlag={setFlag} menu={menu} />}
                </div>
              )}
              {checkClaims(url.pathname, 'put') && (
                <div className="md:w-1/2 w-full px-1">
                  {menu.statusId === 1 && <ModalProcessMenu setFlag={setFlag} menu={menu} />}
                </div>
              )}
              {checkClaims(url.pathname, 'post') && (
                <div className="md:w-1/2 w-full px-1">
                  {(menu.statusId === 1 || menu.statusId === 2) && <ModalCancelMenu setFlag={setFlag} menu={menu} />}
                </div>
              )}
            </div>
          </div>
          <Collapse in={expanded} timeout="auto" unmountOnExit>
            <div className="flex justify-between items-center text-xs px-3 py-1">
              ...
              <div className="w-1/5 text-start">مجموع مبلغ</div>
            </div>
            ...
            <div className="flex justify-between py-2 px-5">
              <div className="text-start px-3 py-1">
                <span className="text-xs">مجموع تعداد سفارش : </span>

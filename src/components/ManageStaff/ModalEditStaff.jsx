...
export default function ModalEditStaff({ handleCloseMenu, setFlag, staff, listBuilding, listYear, open, setOpen }) {
  ...
  useEffect(() => {
    setValBuilding(listBuilding.find((e) => e.id === staff.buildingId));

    if (staff) {
      setFileAtt(staff.image);
      setMobileNumber(staff.userPhoneNumber);
      setAvatarTemporary(`${mainDomain}${staff.avatar}`);
      setIsResident(staff.isFromResident);

      if (listUnit.length > 0) {
        setValUnit(listUnit.find((e) => e.id === staff.unitId));
      }
      if (listResident.length > 0) {
        setValResident(listResident.find((e) => e.userId === staff.userId));
      }
      setValyear(staff.yearId);
      setShowMobileNumber(staff.showMobileNumber);
      setNameFamily(staff.nameFamily);
      setGender(staff.gender);
    }
  }, [staff, listUnit, listResident]);
  ...
  const editStaffHandler = () => {
    if (
      ...
      setIsLoading(true);
      axios
        .put(`${mainDomain}/api/Staff/Update`, data, {
          headers: {
            Authorization: `Bearer ${localStorage.getItem('token')}`,
          },
        })
              ...
              <div className="flex items-center flex-wrap flex-col-reverse md:flex-row">
                <div className="md:w-3/4 w-full md:mt-0 mt-8">
                  ...
                  <div className="flex justify-between flex-wrap">
                    ...
                    <div className=" w-full mt-3 px-1">
                      <SelectServiceStaff valBuilding={valBuilding} setServiceIds={setServiceIds} serviceIdsSelected={staff.serviceIds} />
                    </div>
          ...
          {isResident && (
            <div>
              <div className="flex items-start flex-wrap">
                ...
                <div className="flex justify-between mt-3 w-2/3 flex-wrap">

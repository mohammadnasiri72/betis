...
export default function ModalEditServiceBlock({
  setFlag,
  listService,
  listServiceTime,
  valService,
  ...
  useEffect(() => {
    if (block) {
      ...
      if (block.serviceId) {
        setValService(listService.find((e) => e.id === block.serviceId));
      }
      if (block.serviceTimeId && listServiceTime.length > 0) {
        setServiceTime(listServiceTime.find((e) => e.id === block.serviceTimeId));
      }
      if (block.unitId) {
        if (block.unitId === 0) {
          setValUnit(allUnitsOption);
        } else {
          setValUnit(listUnit.find((e) => e.id === block.unitId));
        }
      }
      if (block.startDateFa) {
        setDateStart(block.startDateFa);
      }
      if (block.endDateFa) {
        setDateEnd(block.endDateFa);
      }
    }
  }, [block]);
  ...
  const editServiceBlock = () => {
    ...
    if ((dateStart && dateEnd) || (!dateStart && !dateEnd)) {
      ...
      setIsLoading(true);
      axios
        .put(`${mainDomain}/api/ServiceBlocking/Update`, data, {
          headers: {
        ...
        .then((res) => {
          ...
          Toast.fire({
            icon: 'success',
            text: 'تحریم با موفقیت ویرایش شد',
            customClass: {
              container: 'toast-modal',
            },
          });
            ...
            <div className="w-full flex-wrap flex ">
              ...
              <div className="flex flex-wrap items-start w-full">
                ...
                <div className=" relative w-1/2 px-1 flex items-center mt-3">
                  {/* <p className="whitespace-nowrap mt-3 px-1">تاریخ شروع: </p> */}
                  <DatePicker
                ...
                <div className=" relative w-1/2 px-1 flex items-center mt-3">
                  {/* <p className="whitespace-nowrap  px-1">تاریخ پایان: </p> */}
                  <DatePicker
                    className={themeMode === 'dark' ? 'bg-dark rmdp-mobile' : 'rmdp-mobile'}

...
export default function ModalNewBasicInfo({ setFlag, listCategory, valCategoryMain }) {
  ...
  const setNewBasicInfoHandler = () => {
    if (title && valCategoryId) {
      ...
      setIsLoading(true);
      axios
        .post(`${mainDomain}/api/BasicInfo/Add`, data, {
          headers: {
            Authorization: `Bearer ${localStorage.getItem('token')}`,
          },
        })
        .then((res) => {
          ...
          Toast.fire({
            icon: 'success',
            text: 'اطلاعات پایه با موفقیت ثبت شد',
            customClass: {
              container: 'toast-modal',
            },
          });
      ...
      >
        ...
        <DialogActions>
          <Button
            ...
            variant="contained"
            autoFocus
            onClick={setNewBasicInfoHandler}
          >
            ثبت
          </Button>
        </DialogActions>

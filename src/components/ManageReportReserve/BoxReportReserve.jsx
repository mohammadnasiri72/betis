...
function BoxReportReserve({ numStatusReserv }) {
  ...
  // محاسبه آمار کلی
  const totalReservations = numStatusReserv.reduce((sum, status) => sum + status.number, 0);
  ...
  const statusInfo = {
    ...
    2: {
      title: 'رد شده',
      icon: <Block className="text-orange-500" />, // تغییر به نارنجی
      color: 'warning',
    ...
    3: {
      title: 'انجام شده',
      icon: <Done className="text-blue-500" />,
      color: 'info',
    ...
    4: {
      title: 'لغو شده',
      icon: <Cancel className="text-red-500" />, // تغییر به قرمز
      color: 'error',
  ...
  const getStatusCount = (statusId) => {
    const status = numStatusReserv.find((item) => item.statusId === statusId);
    return status ? status.number : 0;
  };

  const pendingCount = getStatusCount(0);
  const confirmedCount = getStatusCount(1);
  const rejectedCount = getStatusCount(2);
          ...
          <Box className="flex items-center justify-between mb-4">
            <Box className="flex items-center gap-2">
              <Box className={`p-2 rounded-lg ${isDark ? 'bg-gray-700' : 'bg-purple-50'}`}>
                <MdEventAvailable className={`text-3xl ${isDark ? 'text-purple-400' : 'text-purple-600'}`} />
              </Box>
          ...
          <Grid container spacing={2}>
            ...
            <Grid item xs={12} md={7}>
              ...
              >
                ...
                >
                  <MdEventAvailable className="text-blue-500" fontSize="small" />
                  وضعیت رزروها
                </Typography>

                <Box className="space-y-2">
                  {numStatusReserv.map((status) => {
                          ...
                          <Box className="flex items-center gap-2 !select-none">
                            ...
                            <Box>
                              ...
                              <Typography variant="caption" className="text-gray-500">
                                {status.number > 0
                                  ? `${((status.number / totalReservations) * 100).toFixed(1)}%`
                                  : '۰%'}
                              </Typography>
                            </Box>
                          </Box>
                          <Box className="flex items-center gap-1 !select-none">
                            <span className="text-xs text-[#0008]">{status.number} رزرو</span>
...
export default BoxReportReserve;

...
import { Autocomplete, Box, FormControl, InputAdornment, InputLabel, MenuItem, Paper, Select, Stack, TextField } from '@mui/material';
...
export default function ModalEditPet({ setFlag, listUnit , pet , handleCloseMenu}) {
  const [open, setOpen] = React.useState(false);

  const [isLoading, setIsLoading] = React.useState(false);
  const [typePet, setTypePet] = React.useState(pet.type);
  const [errTypePet, setErrTypePet] = React.useState(false);
  const [namePet, setNamePet] = React.useState(pet.name);
  const [racePet, setRacePet] = React.useState(pet.race);
  const [errRacePet, setErrRacePet] = React.useState(false);
  const [agePet, setAgePet] = React.useState(pet.age);
  const [fileAtt, setFileAtt] = React.useState('');
  const [avatarTemporary, setAvatarTemporary] = React.useState('');
  const [valUnit, setValUnit] = React.useState(listUnit.find((e)=>e.id === pet.unitId));
  ...
  React.useEffect(() => {
    if (pet.avatar) {
        setAvatarTemporary(mainDomain+pet.avatar)
    }
   
    if (pet.image) {
      setFileAtt(pet.image);
    }
  }, [pet]);
  ...
  const EditPetHandler = () => {
    ...
    if (valUnit && typePet && racePet) {
      ...
      setIsLoading(true);
      axios
        .put(`${mainDomain}/api/Pet/Update`, data, {
          headers: {
            Authorization: `Bearer ${localStorage.getItem('token')}`,
          },
        })
        .then((res) => {
          ...
          Toast.fire({
            icon: 'success',
            text: 'حیوان خانگی با موفقیت ویرایش شد',
            customClass: {
              container: 'toast-modal',
            },
          });
            ...
            <div className="sm:w-2/3 w-full">
              ...
              <div className="flex flex-wrap mt-3 items-start">
                ...
                <div className="sm:w-1/2 w-full px-1" dir="rtl">
                  <FormControl focused={errTypePet}
                    color={errTypePet ? 'error' : 'primary'} size="small" className="w-full">

...
export default function ModalDeleteCostIncome({ CostIncome, setFlag }) {
  ...
  //   delete handler
  const deleteCostIncomeHandler = () => {
    setIsLoading(true);
    axios
      .delete(`${mainDomain}/api/Document/Delete/${CostIncome.id}`, {
        headers: {
  ...
  return (
    <>
      <Tooltip title={`لغو ${CostIncome.typeId === 20 ? 'هزینه' : 'درآمد'}`}>
        <IconButton onClick={handleClickOpen}>
          <BsTrash className="text-red-500 text-2xl hover:text-red-600 duration-300" />
        </IconButton>
      </Tooltip>
          ...
          <div className="flex items-center">
            ...
            <div className="text-start px-3">
              <h4 style={{ color: themeMode === 'dark' ? '#fff' : '#000' }}>
                لغو {CostIncome.typeId === 20 ? 'هزینه' : 'درآمد'}
              </h4>
              <p className={themeMode === 'dark' ? 'text-[#fff8]' : 'text-[#0008]'}>آیا از لغو مطمئن هستید؟</p>
            </div>
          </div>
        </DialogContent>

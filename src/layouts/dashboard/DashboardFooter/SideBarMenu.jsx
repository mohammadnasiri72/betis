...
export default function SideBarMenu({ logoImg, open, setOpen, setFlagRefreshPage }) {
  ...
  const StyledToggleButton = styled(ToggleButton)({
    ...
    backgroundColor: themeMode === 'dark' ? 'rgba(44, 48, 68, 0.85) !important' : 'rgba(240, 242, 245, 0.95) !important',
    borderRadius: 14,
    margin: '6px 0',
    minHeight: 48,
    ...
    '&:hover': {
      backgroundColor: themeMode === 'dark' ? 'rgba(60, 65, 100, 0.95) !important' : 'rgba(220, 225, 255, 0.95) !important',
      color: themeMode === 'dark' ? '#fff' : '#4f46e5',
    },
    '&.Mui-selected': {
      color: themeMode === 'dark' ? '#fff' : '#4f46e5',
      backgroundColor: themeMode === 'dark' ? 'rgba(80, 90, 180, 0.95) !important' : 'rgba(200, 210, 255, 0.95) !important',
      fontWeight: 700,
    },
  });
  ...
  const groupedSidebar = [
    ...
    },
    {
      ...
      items: [sidebarResident[1], sidebarResident[2], sidebarResident[3], sidebarResident[4]],
    },
    {
      group: 'مالی',
            ...
            {groupObj.items.length > 1 ? (
              <>
                <StyledListItemButton
                  ...
                  onClick={(e) => { e.stopPropagation(); handleToggleGroup(groupObj.group); }}>
                  <div className='flex w-full justify-between'>
                    <span className="ml-2" style={{ marginLeft: 8 }}>{openGroup[groupObj.group] ? <IoIosArrowUp /> : <IoIosArrowDown />}</span>
                    <div>
